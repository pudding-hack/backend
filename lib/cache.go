...
func InitRedis(cfg RedisConfig) (CacheService, *redis.Pool) {
	redisAddress := fmt.Sprintf("%s:%d", cfg.Host, cfg.Port)
	pool, errPool := CreateRedisPool(redisAddress, cfg.Password, cfg.MaxIdle)
